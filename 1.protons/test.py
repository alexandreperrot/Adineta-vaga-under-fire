...
def loadfile(n):
    with open(f'outputs/P_{n}.csv','rt') as f:
...
def all_hist():
    ...
    for i in range(3,26,10):
        ...
        n=f'Dose_{i}.10E3'
        dose=loadfile(n)
        bins_dose=np.linspace(0,6,20)
        ax[0].hist(dose,bins=bins_dose, histtype='step', linewidth=1.5, alpha=1, label=n)
        ax[0].annotate(xy=(0+(0.1*(i-1)),0.5), xycoords=('data','axes fraction'),text=f'{dose.mean():.2f} $Gy$',
        color='r', bbox=dict(facecolor='None',edgecolor='r') )
    ...
    plt.suptitle('Poisson distribution over #protons (precise)',bbox=dict(facecolor='None',edgecolor='black'))
    plt.tight_layout()
    plt.savefig('P_poisson_precise.png')
